...
export default class BaiTapRenderFilm extends Component {
    ...
    renderProductByMap = () => {
        const arrJSX = mangPhim.map((film, index) => {
            return <div className="film col-4" key={index}>
                <img className="card-img-top" src={film.hinhAnh} alt />
                <div className="card-body">
                    <h1 className="card-title text-center">{film.tenPhim}</h1>
                    <p className="card-text">{film.mangPhim}</p>
                    <p className="card-text">{film.moTa}</p>
                    <p className="card-text text-center">{film.ngayKhoiChieu}</p>
    ...
    renderProductByMap2 = () => {
        ...
        const arrJSX = mangPhim.map((phim,index) => {
            ...
            <div className="card text-white bg-primary">
                ...
                <div className="card-body">
        <h4 className="card-title" style={{height:'50px'}} > {phim.tenPhim.length > 50 ? <span>{phim.tenPhim.substr(0,50)}...</span> : <span>{phim.tenPhim}</span>}</h4>
                    {/* <h4 style={objectStyle} className="card-title">{phim.tenPhim}</h4> */}
                    {/* <h4 style={{
                         width:'100%',
                         height:'300px'
                    }} className="card-title">{phim.tenPhim}</h4> */}
                    <p className="card-text">{phim.moTa.length > 70 ? <span>{phim.moTa.substr(0,70)}...</span> : <span>{phim.moTa}</span>}</p>
                </div>
            </div>
          </div>;
        })
...
// luu y cach import css 
// khi import bình thường thì tất cả các file khác trong app sẽ bị ảnh hưởng nếu đặt cùng tên class -> cách sử dụng thêm đuôi .module 
// import style from './BaiTapRenderFilm.module.css'
//  <img className={`card-img-top ${style.hinhAnhPhim}`} src={phim.hinhAnh} alt />